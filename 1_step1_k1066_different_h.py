...
for collision_type in collision_types:
  ...
  filename = output_folder+'/1_knots_ids_k'+str(k)+'.pkl'
  ...
  def compute_reweighted_kernels(midpoints_ids, knots_ids, h=1000, path_to_jsonl="0_paths_with_legs_ALL_combined.jsonl.gz", sp_lookup_path=None):
  ...
  for h in hs:
    try:
      filename = output_folder+'/1_K_weighted_k'+str(k)+'_h'+str(h)+'.pkl'
      ...
      K_weighted = compute_reweighted_kernels(midpoints_ids, knots_ids, h=h, sp_lookup_path='0_sp_lookups.pkl')
      print("✅ Kernel matrix shape:", K_weighted.shape)
    ...
    # ==============================================
    # Fit NPC Model with Reweighted Kernel
    # ==============================================
    ...
    # Save trace
    az.to_netcdf(trace, output_folder+"/1_trace_k"+str(k)+'_h'+str(h)+'_'+collision_type+'.nc')

    idata = az.from_netcdf(output_folder+"/1_trace_k"+str(k)+'_h'+str(h)+'_'+collision_type+'.nc')
    # WAIC
    waic = az.waic(idata)
    print("WAIC:", waic)

    # Save to file
    with open(output_folder+f"/1_waic_k{k}_h{h}_{collision_type}.pkl", "wb") as f:
        pickle.dump(waic, f)
    ...
    # Save to file
    with open(output_folder+f"/1_loo_k{k}_h{h}_{collision_type}.pkl", "wb") as f:
        pickle.dump(loo, f)
    ...
    summary_stats.to_csv(output_folder+'/1_summary_stats_k'+str(k)+'_h'+str(h)+'_'+collision_type+'.csv')
    print(summary_stats)