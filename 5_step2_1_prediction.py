...
##################
output_folder = '5_NPC_DFS_after_2-1'
...
for collision_type in collision_types:
    ...
    ##for DFS after-period prediction
    dfs_folder = '3_k1066_h1000_step2-1'
    ...
    trace_file = dfs_folder + f'/3_trace_k1066_h1000_{collision_type}_Triangular.nc'

    new_ids = data_dfs['Index'].values.tolist()
    knots_ids_path = dfs_folder + '/3_knots_ids_k1066.pkl'
    ...
    print('collision type:',collision_type,'step 2-1, DFS after-period only model == beta_mean:',beta_mean,'beta0_mean:',beta0_mean)
    print('')

    df_dfs_segs['predicted_after_mcmc'] = trace.posterior["lambda_"].mean(dim=("chain", "draw")).values

    lower_upper = az.hdi(trace, var_names=["lambda_"], hdi_prob=0.95)
    df_dfs_segs['predicted_after_mcmc_2.5%'] = lower_upper['lambda_'].isel(hdi=0)
    df_dfs_segs['predicted_after_mcmc_97.5%'] = lower_upper['lambda_'].isel(hdi=1)
    df_dfs_segs.to_csv(output_folder+ f'/5_DFS_sites_pred_after_2-1_{collision_type}.csv')